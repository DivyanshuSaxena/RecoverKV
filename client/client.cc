...
int KV739Client::kv739_init(char *server_name) {
	...
	try {
		...
		client = new RecoverKVClient(grpc::CreateChannel(server_name, grpc::InsecureChannelCredentials()));
		return 0;
...
int KV739Client::kv739_get(char *key, char *value) {
	...
	if (client == NULL) {
		cout << "[ERROR]" << "Client not initialized" << endl;
		return -1;
	}
...
int KV739Client::kv739_put(char *key, char *value, char *old_value) {
	...
	if ((value != NULL) && (value[0] == '\0')) {
		cout << "[ERROR]" << "New value cannot be empty" << endl;
		return -1;
	}
