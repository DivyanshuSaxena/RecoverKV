...
int test_correctness_write_intensive(KV739Client *client, int total_requests,
  ...
  // send write requests to server
  for (int i = 0; i < total_requests * total_cycles; i++) {
    if (client->kv739_put(keys[i % total_requests], newValues[i],
                          oldValues[i % total_requests]) == -1) {
...
int test_correctness_durability(KV739Client *client, int total_requests) {
  ...
  cout << "TEST 4 [Please restart the server now, waiting for "
       << SLEEP_INTERVAL << " seconds...]" << endl;
